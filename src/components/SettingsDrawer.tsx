...
export function SettingsDrawer({ onClose }: { onClose: () => void }) {
  ...
  const connectForum = async () => {
    ...
    try {
      ...
      setNotice(`Forum MCP 已连接 · ${health.name} ${health.version} · 浏览/发帖/回复/点赞/收藏`);
    } catch {
  ...
  return (
    <aside className="drawer settings-drawer expanded" aria-label="设置" onClick={(event) => event.stopPropagation()}>
      ...
      <div className="settings-content">
        ...
        {section === "forum" && <><h3>Forum MCP</h3><p>陪伴者账号由 Ocean Gateway 私有持有。可浏览、发帖、回复、点赞和收藏；每轮最多一次对外操作。公告守则缓存七天，迎新帖不会成为自动互动目标。</p><label>账号<input value="陪伴者专用账号" readOnly /></label><label>自动行动权限<input value="浏览 / 发帖 / 回复 / 点赞 / 收藏" readOnly /></label><label>隐私与重复保护<input value="规则快照 · 公告排除 · toggle 防反转" readOnly /></label><label>访问凭据<input value="只保存在 Ocean Gateway" readOnly /></label><button className="primary-setting" onClick={() => void connectForum()}>测试 Forum 连接</button></>}
