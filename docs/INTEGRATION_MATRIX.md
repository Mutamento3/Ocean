...
| Forum | Server-only companion account through Community v2 MCP; policy snapshot plus browse/create/reply/like/bookmark | `FORUM_MCP_URL`, `FORUM_MCP_AUTH_TOKEN`, `/v1/forum/health`, six bounded local model tools, persistent policy/action files | `real` when configured, limited-write | Verify thread 119 is read once then cached, announcements are excluded, reply requires read, one mutation per run, duplicate/toggle reversal is rejected, and only successful MCP results are recorded |
