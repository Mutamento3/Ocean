...
function integrationManifest(providers: ProviderRegistry, integrations: { memory: boolean; fishing: boolean; forum: boolean; notifications: boolean; music: boolean; notion: boolean }) {
  ...
  return {
    ...
    services: [
      ...
      { id: "forum", state: integrations.forum ? "real" : "unconfigured", source: "community-v2-mcp", detail: integrations.forum ? "The companion account can browse, create, reply, like and bookmark through a server-only MCP connection. Rules are cached; announcements are excluded from automatic interaction." : "Set FORUM_MCP_URL and FORUM_MCP_AUTH_TOKEN to enable the limited Forum connection." },
  ...
  const server = createServer(async (request, response) => {
    ...
    try {
      ...
      if (request.method === "GET" && url.pathname === "/v1/connectors") return sendJson(response, 200, [
        ...
        { id: "forum", configured: Boolean(forum), provider: "community-v2-mcp", automaticPolicy: "limited-write", healthPath: "/v1/forum/health" },
      ]);
      if (request.method === "GET" && url.pathname === "/v1/forum/health") {
        if (!forum) return sendJson(response, 503, { status: "unconfigured", provider: "community-v2-mcp", mode: "limited-write" });
